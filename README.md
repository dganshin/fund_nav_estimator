...
```bash
FUND_NAV_QUOTE_WORKERS=12 uvicorn src.frontend_app:app --reload --host 127.0.0.1 --port 8502
```

`FUND_NAV_QUOTE_WORKERS` 控制实时行情并发线程数，默认 12，范围 1-24。自选很多时可调大；如果行情源限流或本机网络不稳，可调回 6。
...
- 自选基金显示估值但无盈亏
- 基金列表附带 **盘中分时涨跌缩略图 (Sparkline)**，一瞥即可了解走势
- 支持极致实时的高频自动刷新（1/2/5/10/30秒可选）
...
- 面板操作：修改总金额、加仓、减仓、清仓
- 动态图表切换：
  - **盘中实时 (Intraday)**：以百分比或金额绘制当天的分时走势图（午休自动断开分割）。
  - **近30日 (Historical)**：估值 vs 真实净值对比图、误差趋势图。
- 手动校准 / 强制重演校准
...
- 主动基金调仓会影响准确度
- 公开持仓可能滞后（季报延迟）
- QDII/海外基金走独立观察榜, 用于夜盘/次日早上收益预估, 不参与 14:50 A股决策榜
- 白银期货等商品型基金无股票持仓，不适用
- 不接入实盘交易
